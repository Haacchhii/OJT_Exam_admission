...
import { formatTime, badgeClass } from '../../utils/helpers.js';
...
export default function EmployeeExams() {
  ...
  const bump = () => refresh(v => v + 1);

  return (
    <div>
      <div className="flex gap-2 mb-6 flex-wrap">
        {[['exams','📝 Exams'],['builder','➕ Create Exam'],['schedules','📅 Schedules']].map(([k,l]) => (
          <button key={k} onClick={() => { if (k !== 'builder') setEditExamData(null); setTab(k); }} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${tab === k ? 'bg-[#166534] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{l}</button>
        ))}
      </div>
      {tab === 'exams' && <ExamsList onEdit={(exam) => { setEditExamData(exam); setTab('builder'); }} bump={bump} />}
      {tab === 'builder' && <ExamBuilder editExam={editExamData} onDone={() => { setEditExamData(null); setTab('exams'); bump(); }} />}
      {tab === 'schedules' && <ScheduleManager bump={bump} />}
    </div>
  );
}

/* ===== EXAMS LIST ===== */
function ExamsList({ onEdit, bump }) {
        ...
        <div className="lpu-card p-6 mb-4">
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => onEdit(exam)} className="bg-[#166534] text-white px-3 py-1.5 rounded-lg text-sm font-medium hover:bg-[#14532d]">✏️ Edit</button>
              <button onClick={() => { updateExam(exam.id, { isActive: !exam.isActive }); refresh(v => v + 1); }} className="border border-gray-300 text-gray-700 px-3 py-1.5 rounded-lg text-sm hover:bg-gray-50">{exam.isActive ? 'Deactivate' : 'Activate'}</button>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <D label="Grade Level" v={exam.gradeLevel} /><D label="Duration" v={`${exam.durationMinutes} minutes`} />
            <D label="Passing Score" v={`${exam.passingScore}%`} /><D label="Questions" v={exam.questions.length} />
            <D label="Schedules" v={eSched.length} /><D label="Registrations" v={`${eRegs.length} (${completed} completed)`} />
          </div>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {eRegs.map(r => {
                    const allU = getUsers();
                    const student = allU.find(u => u.email === r.userEmail);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard icon="📝" value={exams.length} label="Total Exams" color="blue" />
        <StatCard icon="📅" value={schedules.length} label="Schedules" color="emerald" />
        <StatCard icon="👥" value={regs.length} label="Registrations" color="amber" />
        <StatCard icon="✅" value={regs.filter(r => r.status === 'done').length} label="Completed" color="amber" />
      </div>

      {/* Exams filter bar */}
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <input value={searchExam} onChange={e => { setSearchExam(e.target.value); resetExamPage(); }} placeholder="Search exams…" className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#166534]/20 outline-none text-sm" />
      ...
      <div className="lpu-card p-4 mb-6">
        {paginatedExams.length > 0 ? (
                    ...
                    <tr key={e.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-2">
                        <div className="flex gap-1">
                          ...
                          <button onClick={() => { updateExam(e.id, { isActive: !e.isActive }); refresh(v => v + 1); }} className="text-gray-500 hover:underline text-xs">{e.isActive ? 'Deactivate' : 'Activate'}</button>
                          <button onClick={async () => { if (await confirm({ title: 'Delete Exam', message: 'Are you sure you want to delete this exam? This cannot be undone.', confirmLabel: 'Delete', variant: 'danger' })) { deleteExam(e.id); refresh(v => v + 1); bump(); } }} className="text-red-500 hover:underline text-xs">Delete</button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <EmptyState icon="📝" title="No exams found" text="No exams match your filters." />
        )}
      </div>
      ...
      <div className="lpu-card p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <input value={readSearch} onChange={e => { setReadSearch(e.target.value); resetReadPage(); }} placeholder="Search student…" className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#166534]/20 outline-none text-sm" />
        ...
        {paginatedReadiness.length > 0 ? (
          ...
          <EmptyState icon="📋" title="No registrations found" text="No exam registrations match your filters." />
        )}
      </div>
    </div>
  );
...
function parseCSVQuestions(text) {
  ...
  for (const line of lines) {
    ...
    if (type === 'essay') {
      parsed.push({ id: Date.now() + parsed.length, questionText: qText, questionType: 'essay', points: pts, orderNum: parsed.length + 1, choices: [] });
      ...
      parsed.push({
        ...
        choices: choiceTexts.map((t, i) => ({ id: Date.now() + parsed.length * 10 + i, choiceText: t, isCorrect: i === correctIdx })),
      });
    } else { errs++; }
  }
  return { parsed, errs };
  ...
  for (const item of items) {
    ...
    if (type === 'essay') {
      ...
      const choices = rawChoices.map((c, i) => {
        ...
        return { id: Date.now() + parsed.length * 10 + i, choiceText: text, isCorrect };
      });
      parsed.push({ id: Date.now() + parsed.length, questionText: qText, questionType: 'mc', points: pts, orderNum: parsed.length + 1, choices });
    } else { errs++; }
  }
  return { parsed, errs };
}
...
function ExamBuilder({ editExam, onDone }) {
  ...
  const addQuestion = () => {
    ...
    const q = { id: Date.now(), questionText: qText, questionType: qModal, points: pts, orderNum: questions.length + 1, choices: [] };
    if (qModal === 'mc') {
      q.choices = choices.filter(c => c.text.trim()).map((c, i) => ({ id: Date.now() + i, choiceText: c.text, isCorrect: !!c.correct }));
    }
  ...
  const confirmUpload = (mode) => {
    ...
    if (mode === 'replace') {
      ...
      setQuestions(prev => {
        const merged = [...prev, ...uploadPreview.parsed.map((q, i) => ({ ...q, id: Date.now() + i, orderNum: prev.length + i + 1 }))];
        return merged;
      });
    }
  ...
  const saveExam = () => {
    ...
    if (editExam) {
      updateExam(editExam.id, { title, gradeLevel: grade, durationMinutes: parseInt(duration), passingScore: parseInt(passing), questions });
      showToast('Exam updated!', 'success');
    } else {
      addExam({ title, gradeLevel: grade, durationMinutes: parseInt(duration), passingScore: parseInt(passing), questions });
    ...
    <div>
      ...
      <div className="lpu-card p-6 mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Inp label="Exam Title" value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Entrance Exam - Grade 7" />
          <div>
          ...
          <Inp label="Duration (minutes)" type="number" value={duration} onChange={e => setDuration(e.target.value)} placeholder="60" />
          <Inp label="Passing Score (%)" type="number" value={passing} onChange={e => setPassing(e.target.value)} placeholder="60" />
        </div>
      </div>
      ...
      <div className="lpu-card p-6 mb-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h3 className="text-lg font-bold text-forest-500">📤 Upload Questionnaire</h3>
            <p className="text-gray-500 text-sm mt-0.5">Import questions from a CSV or JSON file to quickly build your exam.</p>
          </div>
          <div className="flex gap-2">
            <button onClick={() => downloadTemplate('csv')} className="text-xs border border-gray-300 text-gray-600 px-2.5 py-1.5 rounded-lg hover:bg-gray-50 flex items-center gap-1">📄 CSV Template</button>
            <button onClick={() => downloadTemplate('json')} className="text-xs border border-gray-300 text-gray-600 px-2.5 py-1.5 rounded-lg hover:bg-gray-50 flex items-center gap-1">📄 JSON Template</button>
          </div>
        </div>
        ...
        <details className="mt-4 group">
          <summary className="text-sm text-forest-500 font-medium cursor-pointer hover:underline select-none">ℹ️ How to format your file</summary>
  ...
  { "type": "essay", "question": "...",
    "points": 5 }
      ...
      <Modal open={!!uploadPreview} onClose={() => setUploadPreview(null)}>
        {uploadPreview && (
          <div>
            <h3 className="text-lg font-bold text-forest-500 mb-2">📋 Import Preview</h3>
            ...
            {questions.length > 0 ? (
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  <button onClick={() => confirmUpload('append')} className="bg-[#166534] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#14532d]">➕ Add to Existing</button>
                  <button onClick={() => confirmUpload('replace')} className="bg-forest-500 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-forest-600">🔄 Replace All</button>
              ...
              <div className="flex gap-2">
                <button onClick={() => confirmUpload('replace')} className="bg-forest-500 text-white px-5 py-2 rounded-lg font-semibold hover:bg-forest-600">✅ Import Questions</button>
      ...
      <div className="lpu-card p-6 mb-4">
        ...
        {questions.length > 0 ? (
          ...
          <EmptyState icon="📝" title="No Questions Added" text="Upload a questionnaire file above, or click the buttons to add questions manually." />
        )}
      </div>

      <div className="flex gap-3">
        <button onClick={saveExam} className="bg-forest-500 text-white px-6 py-2.5 rounded-lg font-semibold hover:bg-forest-600 shadow-md">💾 Save Exam</button>
...
function ScheduleManager({ bump }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (editId) { updateExamSchedule(editId, data); showToast('Schedule updated!', 'success'); setEditId(null); }
    else { addExamSchedule(data); showToast('Schedule added!', 'success'); }
    ...
    <div>
      ...
      <div className="lpu-card p-6 mb-6">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <Inp label="Date" type="date" value={form.date} onChange={set('date')} required />
          <Inp label="Start Time" type="time" value={form.start} onChange={set('start')} required />
          <Inp label="End Time" type="time" value={form.end} onChange={set('end')} required />
          <Inp label="Max Applicants" type="number" value={form.slots} onChange={set('slots')} placeholder="30" required />
      ...
      <div className="lpu-card p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <input value={schedSearch} onChange={e => { setSchedSearch(e.target.value); resetSchedPage(); }} placeholder="Search by exam or date…" className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#166534]/20 outline-none text-sm" />
          ...
          {paginatedScheds.map(s => {
            ...
            return (
              <div key={s.id} className="flex items-center gap-4 bg-gray-50 rounded-lg p-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => editSched(s)} className="text-[#166534] hover:underline text-xs">✏️ Edit</button>
                  <button onClick={async () => { if (await confirm({ title: 'Delete Schedule', message: 'Are you sure you want to delete this schedule?', confirmLabel: 'Delete', variant: 'danger' })) { deleteExamSchedule(s.id); refresh(v => v + 1); bump(); } }} className="text-red-500 hover:underline text-xs">🗑 Delete</button>
                </div>
              </div>
            );
          })}
          {paginatedScheds.length === 0 && <EmptyState icon="📅" title="No schedules" text="No schedules match your filters." />}
        </div>
...
function D({ label, v }) { return <div><span className="block text-xs text-gray-400 uppercase tracking-wide">{label}</span><span className="text-sm text-forest-500 font-medium">{String(v)}</span></div>; }
function Inp({ label, required, ...props }) { return <div><label className="block text-sm font-medium text-gray-700 mb-1">{label}</label><input {...props} required={required} className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#166534]/20 outline-none" /></div>; }
