...
} from '../../api/academicYears.js';
import { showToast } from '../../components/Toast.jsx';
import { PageHeader, Badge, EmptyState, ErrorAlert } from '../../components/UI.jsx';
import Icon from '../../components/Icons.jsx';
import Modal from '../../components/Modal.jsx';
import { useConfirm } from '../../components/ConfirmDialog.jsx';
import { SEMESTER_NAMES } from '../../utils/constants.js';

const emptyYearForm  = { year: '', isActive: false };
const emptySemForm   = { name: 'First Semester', academicYearId: '', isActive: false };

export default function EmployeeSettings() {
  const { data: academicYears, loading: ayLoading, error: ayError, refetch: refetchAY } = useAsync(() => getAcademicYears());
  const { data: allSemesters, loading: semLoading, error: semError, refetch: refetchSem } = useAsync(() => getSemesters());
  ...
  const displayedYear = selectedYearId
  ...
  const handleSaveSem = async () => {
    ...
    try {
      if (editSemId) {
        await updateSemester(editSemId, semForm);
  ...
  if (ayError || semError) return (
    <div className="space-y-8">
      <PageHeader title="Settings" subtitle="Manage academic years and semesters" />
      <ErrorAlert message={(ayError || semError).message} />
    </div>
  );
    ...
    <div className="space-y-8">
      ...
      <section className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          ...
          <button
            onClick={openAddYear}
            className="flex items-center gap-1.5 text-sm font-medium text-white bg-forest-600 hover:bg-forest-700 px-3 py-1.5 rounded-lg transition-colors"
          >
        ...
        {years.length === 0 ? (
          <EmptyState icon="calendar" title="No school years" message="Add a school year to get started" />
        ) : (
            ...
            {years.map(y => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  {y.isActive && <Badge color="green">Active</Badge>}
                </div>
                <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                  <button
                    onClick={() => { setSelectedYearId(y.id); }}
                    className="text-xs text-forest-600 hover:text-forest-800 px-2 py-1 rounded hover:bg-forest-50 transition-colors"
      ...
      <section className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        {semesters.length === 0 ? (
          <EmptyState icon="cog" title="No semesters" message={selectedYearId ? "Add a semester for this school year" : "Select a school year or add one first"} />
        ) : (
          <div className="divide-y divide-gray-100">
            {semesters.map(s => {
              const yearLabel = years.find(y => y.id === s.academicYearId)?.year || '';
