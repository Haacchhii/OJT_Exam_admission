import { SCHOOL_NAME, SCHOOL_LOGO_PATH } from '../../../utils/constants';
...
export function printPdfReport(
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>${escapeHtml(title)}</title><style>@page{size:landscape;margin:10mm}body{font-family:Arial,Helvetica,sans-serif;margin:0;color:#0f172a}.sheet{padding:16px 20px}header.report{display:flex;justify-content:space-between;align-items:flex-end;gap:16px;padding-bottom:10px;border-bottom:2px solid #1f4f73}.brand{display:flex;align-items:center;gap:10px}.brand-logo{width:40px;height:40px;object-fit:contain;flex:0 0 auto}.brand-copy{display:flex;flex-direction:column}.school{font-size:18px;font-weight:700;line-height:1.2;color:#0f172a}.title{font-size:13px;font-weight:600;color:#1f4f73;margin-top:2px}.meta-col{display:flex;flex-direction:column;gap:2px;text-align:right}.meta-item{font-size:10px;color:#475569}.filters{margin:10px 0 4px}.filters-label{font-size:10px;font-weight:700;color:#475569;margin-bottom:4px;text-transform:uppercase;letter-spacing:.04em}</style></head><body><div class="sheet"><header class="report"><div class="brand"><img class="brand-logo" src="${escapeHtml(SCHOOL_LOGO_PATH)}" alt="School logo" /><div class="brand-copy"><div class="school">${escapeHtml(SCHOOL_NAME)}</div><div class="title">${escapeHtml(title)}</div></div></div><div class="meta-col"><span class="meta-item">Generated: ${escapeHtml(formatManilaDateTime(new Date()))}</span><span class="meta-item">Rows exported: ${totalRows}</span></div></header><div class="filters"><div class="filters-label">Applied Filters</div><div>${filtersHtml}</div></div>${sectionsHtml}</div></body></html>`);
  w.document.close();
