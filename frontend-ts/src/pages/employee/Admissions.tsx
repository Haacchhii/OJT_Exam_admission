...
export default function EmployeeAdmissions() {
  ...
  const admissions = useMemo(() => {
    ...
    if (yearFilter !== 'all') list = list.filter(a => (a as any).academicYear?.id === Number(yearFilter));
    if (semesterFilter !== 'all') list = list.filter(a => (a as any).semester?.id === Number(semesterFilter));
      ...
      <div>
        ...
        <div className="gk-card p-6 mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <D label="Student Number" value={(adm as any).studentNumber || (adm.applicantType === 'New' || adm.applicantType === 'Transferee' ? 'Will be assigned on acceptance' : 'N/A')} />
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 text-left text-gray-400 uppercase text-xs">
                    <th scope="col" className="py-3 px-2 w-8"><input type="checkbox" checked={paginated.length > 0 && paginated.every((a: Admission) => selected.has(a.id))} onChange={toggleAll} className="accent-forest-500 rounded" /></th>
