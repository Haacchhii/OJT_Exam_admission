  ...
  return (
    <div className="gk-card p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <Input label="Student First Name" value={form.firstName} onChange={set('firstName')} required placeholder="Juan" />
        <Input label="Student Last Name" value={form.lastName} onChange={set('lastName')} required placeholder="Dela Cruz" />
        ...
        <Input label="Place of Birth" value={form.placeOfBirth} onChange={set('placeOfBirth')} required placeholder="City/Municipality, Province" />
        <Input label="Religion" value={form.religion} onChange={set('religion')} placeholder="Optional" />
        <Input label="Contact Number" type="tel" value={form.phone} onChange={(e: ChangeEvent<HTMLInputElement>) => { const v = e.target.value.replace(/[^0-9+\-\s()]/g, ''); setForm(f => ({ ...f, phone: v })); }} required placeholder="+63 9XX XXX XXXX" />
      </div>
      <div className="mb-6">
        <TextArea label="Complete Address" value={form.address} onChange={set('address')} required placeholder="Street, Barangay, Municipality/City, Province, ZIP Code" rows={3} />
      </div>
