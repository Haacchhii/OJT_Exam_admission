...
export const createAdmissionSchema = z.object({
  ...
  placeOfBirth: z.string().min(1, 'Place of birth is required').max(200),
  ...
  lrn: z.string().min(12, 'LRN must be exactly 12 digits').max(12).regex(/^\d{12}$/, 'LRN must be numeric'),
  applicantType: z.enum(['New', 'Transferee', 'Returning', 'Continuing']).default('New'),
  studentNumber: z.string().max(50).optional().nullable(),
  fatherNameOccupation: z.string().min(1, 'Father\'s name & occupation is required').max(200),
  motherNameOccupation: z.string().min(1, 'Mother\'s name & occupation is required').max(200),
  guardian: z.string().max(200).optional().nullable(),
