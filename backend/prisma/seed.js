...
async function main() {
  ...
  // ─── We need to look up auto-generated question/choice IDs ──
  ...
  // Build a question map by orderNum for exam 1
  const qMap = {};
  for (const q of exam1Questions) {
    qMap[q.orderNum] = q;
  }

  // ─── Submitted Answers (Reg 1 — Maria) ───────────
  // Maria answered: Q1→Manila(choice2), Q2→132(choice2), Q3→Mars(choice3), Q4→Kalayaan(choice1),
  // Q5→Rizal(choice2), Q6→5/4(choice2), Q7→Skin(choice3), Q8→Coal(choice1), Q9→7(choice2), Q10→essay
  const reg1Answers = [
    { orderNum: 1, choiceOrder: 2 },   // Manila ✓
    { orderNum: 2, choiceOrder: 2 },   // 132 ✓
    { orderNum: 3, choiceOrder: 3 },   // Mars ✓
    { orderNum: 4, choiceOrder: 1 },   // Kalayaan ✓
    { orderNum: 5, choiceOrder: 2 },   // Jose Rizal ✓
    { orderNum: 6, choiceOrder: 2 },   // 5/4 ✓
    { orderNum: 7, choiceOrder: 3 },   // Skin ✓
    { orderNum: 8, choiceOrder: 1 },   // Coal ✗
    { orderNum: 9, choiceOrder: 2 },   // 7 ✓
  ];

  for (const a of reg1Answers) {
    const q = qMap[a.orderNum];
    const choice = q.choices[a.choiceOrder - 1];
    await prisma.submittedAnswer.create({
      data: { registrationId: 1, questionId: q.id, selectedChoiceId: choice.id },
    });
  }
  // Essay answer for Q10
  const q10 = qMap[10];
  await prisma.submittedAnswer.create({
    data: { registrationId: 1, questionId: q10.id, essayText: 'Education is the foundation of a progressive society. It empowers individuals with knowledge and critical thinking skills...' },
  });

  // ─── Submitted Answers (Reg 2 — Ana) ─────────────
  const reg2Answers = [
    { orderNum: 1, choiceOrder: 2 },   // Manila ✓
    { orderNum: 2, choiceOrder: 2 },   // 132 ✓
    { orderNum: 3, choiceOrder: 3 },   // Mars ✓
    ...
    { orderNum: 6, choiceOrder: 2 },   // 5/4 ✓
    { orderNum: 7, choiceOrder: 3 },   // Skin ✓
    { orderNum: 8, choiceOrder: 3 },   // Solar Energy ✓
    { orderNum: 9, choiceOrder: 2 },   // 7 ✓
  ];

  for (const a of reg2Answers) {
    const q = qMap[a.orderNum];
    const choice = q.choices[a.choiceOrder - 1];
    await prisma.submittedAnswer.create({
      data: { registrationId: 2, questionId: q.id, selectedChoiceId: choice.id },
    });
  }
  await prisma.submittedAnswer.create({
    data: { registrationId: 2, questionId: q10.id, essayText: 'I believe education is very important because it helps us learn new things and prepares us for the future...' },
  });
  console.log('  ✅ Submitted answers');

  // ─── Essay Answers ────────────────────────────────
  await prisma.essayAnswer.create({
    data: { registrationId: 1, questionId: q10.id, essayResponse: 'Education is the foundation of a progressive society. It empowers individuals with knowledge and critical thinking skills...', pointsAwarded: 12, maxPoints: 15, scored: true, scoredById: 1 },
  });
  await prisma.essayAnswer.create({
    data: { registrationId: 2, questionId: q10.id, essayResponse: 'I believe education is very important because it helps us learn new things and prepares us for the future...', maxPoints: 15, scored: false },
  });
  console.log('  ✅ Essay answers');
